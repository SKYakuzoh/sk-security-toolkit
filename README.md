...
| Langage | Pré-requis |
|---------|-----------|
| Bash | outils standards Linux ; `subfinder`, `curl` pour sk-enumpassive |
| Python 3 | `colorama`/`requests` (voir `requirements.txt`) ; `nmap`+`sudo` pour sk-recon (+ `whatweb`/`feroxbuster`/`ffuf`/seclists pour les checks web) |
